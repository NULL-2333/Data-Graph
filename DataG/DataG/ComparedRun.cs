...
namespace DataG
{
    public partial class ComparedRun : Form
    {
        ...
        double sectionPoint0 = 0;
        double sectionPoint1 = 0.25;//user can choose the start & end
        double sectionPoint2 = 0.5;
        double sectionPoint3 = 0.75;
        double sectionPoint4 = 1.00;

        int line1Point0 = 0; //start point
        int line1Point1 = 0;
        int line1Point2 = 0;
        int line1Point3 = 0;
        int line1Point4 = 0; //end point

        int line2Point0 = 0; //start point
        int line2Point1 = 0;
        int line2Point2 = 0;
        int line2Point3 = 0;
        ...
        double disA_real(int i)
        {
            double dis = 0;
            for (int j = 1; j < i; j++)
            {
                dis += Math.Sqrt((glpx[j] - glpx[j - 1]) * (glpx[j] - glpx[j - 1]) + (glpy[j] - glpy[j - 1]) * (glpy[j] - glpy[j - 1]));

            }
            return dis;

        }
        double disB_real(int i)
        {
            double dis = 0;
            for (int j = 1; j < i; j++)
            {
                dis += Math.Sqrt((glpx2[j] - glpx2[j - 1]) * (glpx2[j] - glpx2[j - 1]) + (glpy2[j] - glpy2[j - 1]) * (glpy2[j] - glpy2[j - 1]));
        ...
        private void resetButton_Click(object sender, EventArgs e)
        {
            if (fileOpen == true)
            {
                ...
                if (firstPlayFlag == false)
                {
                    ...
                    sensorChart.ChartAreas.Remove(sensorChart.ChartAreas["Vertical1"]);
                    sensorChart.Series.Remove(sensorChart.Series["caS1"]);
                    sensorChart.ChartAreas.Remove(sensorChart.ChartAreas["Vertical2"]);
                    sensorChart.Series.Remove(sensorChart.Series["caS2"]);
                    sensorChart.ChartAreas.Remove(sensorChart.ChartAreas["Vertical3"]);
                    sensorChart.Series.Remove(sensorChart.Series["caS3"]);
                    sensorChart.ChartAreas.Remove(sensorChart.ChartAreas["Vertical4"]);
                    sensorChart.Series.Remove(sensorChart.Series["caS4"]);
                }
            ...
            if (isBitNormalCre == false)
            {
                ...
                for (int i = 0; i < dtrNum - 1; i++)
                {
                    if (coneX[i] != -1 || coneY[i] != -1)
                    {
                        ...
                        pc.X = (float)coneX[i];
                        pc.Y = (float)coneY[i];
                        g.FillEllipse(Brushes.Orange, pc.X, pc.Y, 5, 5);
                    }
                }
                isBitNormalCre = true;
        ...
        private void segmentationButton_Click(object sender, EventArgs e)
        {
            ...
            sectionPoint4 = (double)barpos4 / 100;

            line1Point0 = findLeftNear(sectionPoint0 * distance1, disA, dtrNum);
            ...
            line1Point4 = findLeftNear(sectionPoint4 * distance1, disA, dtrNum);

            line2Point0 = findLeftNear(sectionPoint0 * distance2, disB, dtrNum2);
            ...
            for (int i = 0; i < dtrNum - 1; i++)
            {
                if (coneX[i] != -1 || coneY[i] != -1)
                {
                    ...
                    pc1.X = (float)coneX[i];
                    pc1.Y = (float)coneY[i];
                    g4.FillEllipse(Brushes.Orange, pc1.X, pc1.Y, 5, 5);
                }
            }
        ...
        private void firstTrackBar_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (barpos1 < barpos2 && barpos1 > barpos0)
            {
                ...
                Pen pen0 = new Pen(Brushes.LightGray, 2);
                Pen pen00 = new Pen(Brushes.Gray, 2);
                PointF p11 = new PointF();
                PointF p22 = new PointF();
                if (fileOpen == false) return;
                ...
                if (line2Point0 == 0 && sectionPoint0 != 0)
                {
                    line2Point0 = dtrNum2 - 1;
                }
                ...
                if (line1Point4 == 0 && sectionPoint4 != 0)
                {
                    line1Point4 = dtrNum - 1;
                }
                if (line2Point4 == 0 && sectionPoint4 != 0)
                {
                    line2Point4 = dtrNum2 - 1;
                }
                for (int i = 0; i < line1Point0 - 1; i += 1)
                {
                    p11 = new PointF((float)x[i], (float)y[i]);
                    p22 = new PointF((float)x[i + 1], (float)y[i + 1]);
                    g3.DrawLine(pen0, p11, p22);
                    g4.DrawLine(pen0, p11, p22);
                }
                for (int i = line1Point0; i < line1Point1 - 1; i += 1)
                {
                    p11 = new PointF((float)x[i], (float)y[i]);
                    p22 = new PointF((float)x[i + 1], (float)y[i + 1]);
                ...
                for (int i = 0; i < line2Point0 - 1; i += 1)
                {
                    p11 = new PointF((float)x2[i], (float)y2[i]);
                    p22 = new PointF((float)x2[i + 1], (float)y2[i + 1]);
                    g3.DrawLine(pen00, p11, p22);
                    g4.DrawLine(pen00, p11, p22);
                }
                for (int i = line2Point0; i < line2Point1 - 1; i += 1)
                {
                    p11 = new PointF((float)x2[i], (float)y2[i]);
                    p22 = new PointF((float)x2[i + 1], (float)y2[i + 1]);
                ...
                for (int i = line1Point2; i < line1Point3 - 1; i += 1)
                {
                    p11 = new PointF((float)x[i], (float)y[i]);
                    p22 = new PointF((float)x[i + 1], (float)y[i + 1]);
                    g3.DrawLine(pen1, p11, p22);
                ...
                for (int i = line2Point2; i < line2Point3 - 1; i += 1)
                {
                    p11 = new PointF((float)x2[i], (float)y2[i]);
                    p22 = new PointF((float)x2[i + 1], (float)y2[i + 1]);
                    g3.DrawLine(pen2, p11, p22);
                ...
                for (int i = line1Point3; i < line1Point4 - 1; i += 1)
                {
                    p11 = new PointF((float)x[i], (float)y[i]);
                    p22 = new PointF((float)x[i + 1], (float)y[i + 1]);
                    g3.DrawLine(pen3, p11, p22);
                ...
                for (int i = line2Point3; i < line2Point4 - 1; i += 1)
                {
                    p11 = new PointF((float)x2[i], (float)y2[i]);
                    p22 = new PointF((float)x2[i + 1], (float)y2[i + 1]);
                    g3.DrawLine(pen4, p11, p22);
                ...
                for (int i = line1Point4; i < dtrNum - 1; i += 1)
                {
                    p11 = new PointF((float)x[i], (float)y[i]);
                    p22 = new PointF((float)x[i + 1], (float)y[i + 1]);
                    g3.DrawLine(pen0, p11, p22);
                ...
                for (int i = line2Point4; i < dtrNum2 - 1; i += 1)
                {
                    p11 = new PointF((float)x2[i], (float)y2[i]);
                    p22 = new PointF((float)x2[i + 1], (float)y2[i + 1]);
                    g3.DrawLine(pen00, p11, p22);
        ...
        bool haveReset2 = true;
        bool haveReset3 = true;
        bool haveReset4 = true;
        ...
        private void Section2PictureBox_Click(object sender, EventArgs e)
        {
            ...
            {
                ChartArea ca = new ChartArea();
                ca = sensorChart.ChartAreas.Add("Vertical2");
                ca.BackColor = Color.Transparent;
                ca.BorderColor = Color.Transparent;
                ca.Position.FromRectangleF(sensorChart.ChartAreas[0].Position.ToRectangleF());
                ca.InnerPlotPosition.FromRectangleF(sensorChart.ChartAreas[0].InnerPlotPosition.ToRectangleF());
                ...
                Series sCopy2 = sensorChart.Series.Add("caS2");
                sCopy2.ChartType = sensorChart.Series[0].ChartType;
                foreach (DataPoint point in sensorChart.Series[0].Points)
                {
                    sCopy2.Points.AddXY(point.XValue, point.YValues[0]);
        ...
        private void Section3PictureBox_Click(object sender, EventArgs e)
        {
            ...
            {
                ChartArea ca = new ChartArea();
                ca = sensorChart.ChartAreas.Add("Vertical3");
                ca.BackColor = Color.Transparent;
                ca.BorderColor = Color.Transparent;
                ca.Position.FromRectangleF(sensorChart.ChartAreas[0].Position.ToRectangleF());
                ca.InnerPlotPosition.FromRectangleF(sensorChart.ChartAreas[0].InnerPlotPosition.ToRectangleF());
                ...
                Series sCopy2 = sensorChart.Series.Add("caS3");
                sCopy2.ChartType = sensorChart.Series[0].ChartType;
                foreach (DataPoint point in sensorChart.Series[0].Points)
                {
                    sCopy2.Points.AddXY(point.XValue, point.YValues[0]);
        ...
        private void Section4PictureBox_Click(object sender, EventArgs e)
        {
            ...
            {
                ChartArea ca = new ChartArea();
                ca = sensorChart.ChartAreas.Add("Vertical4");
                ca.BackColor = Color.Transparent;
                ca.BorderColor = Color.Transparent;
                ca.Position.FromRectangleF(sensorChart.ChartAreas[0].Position.ToRectangleF());
                ca.InnerPlotPosition.FromRectangleF(sensorChart.ChartAreas[0].InnerPlotPosition.ToRectangleF());
                ...
                Series sCopy2 = sensorChart.Series.Add("caS4");
                sCopy2.ChartType = sensorChart.Series[0].ChartType;
                foreach (DataPoint point in sensorChart.Series[0].Points)
                {
                    sCopy2.Points.AddXY(point.XValue, point.YValues[0]);
        ...
        private void section2Timer_Tick(object sender, EventArgs e)
        {
            ...
            if (flagPlace == true)
            {
                ...
                double xLeft = dataTime[xLeftSub], xRight = dataTime[xRightSub];
                //two points:A(xLeft,datY[xLeftSub]),B(xRight,datY[xRightSub])
                double xx2 = newPlace + xScale / 2 + moveSpeed + dataTime2[line2Point1];
